...
% @
%
% <<naor, echo=FALSE, fig.cap="Recruitment model with covariates. Using the NAO index as a recruitment index.", eval=FALSE>>=
...
% @
%
% <<naor2, echo=FALSE, fig.cap="Recruitment model with covariates. Using the NAO index as a covariate for the stock-recruitment model parameters.", eval=FALSE>>=
% xyplot(data~year, groups=qname, data=flqs, type="l",
%        auto.key=list(points=FALSE, lines=TRUE, columns=2),
%        par.settings=list(superpose.line=list(col=c("gray35", "black")),
%        strip.background=list(col="gray90")), ylab="")
...
% The previous methods were demonstrated using the maximum likelihood estimation method. However, `ADMB` can also use MCMC methods to fit the model. This section shows how the `sca` and `sca` methods interface with ADMB to use the MCMC fits.
%
...
% <<>>=
% # fit the model
% fitmc1 <- sca(ple4, ple4.indices, fit="MCMC", mcmc=mc)
...
% fitmc2 <- sca(ple4, ple4.indices, fit="MCMC", mcmc=mc)
% @
%
% All fits together
%
